...
def parse_csv_and_add_images():
    ...
    global ENABLE_FOR_PC
    global ENABLE_PICKLE
    
    if ENABLE_PICKLE:
        images_exists = os.path.isfile('images.p')
        meas_exists = os.path.isfile('measurements.p')
    
        if images_exists and meas_exists:
            images = pickle.load(open("images.p", "rb"))
            measurements = pickle.load(open("measurements.p", "rb"))
...
def create_model(samples, type='LeNet'):
    ...
    hist_object = model.fit_generator(train_generator, samples_per_epoch=len(train_samples),
                        validation_data = validation_generator, nb_val_samples = len(validation_samples),
                        nb_epoch = num_of_epochs, verbose=1)
...
history_obj = create_model(samples, type='Nvidia')
visualize_loss(history_obj)
